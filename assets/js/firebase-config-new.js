...
async function initializeFirebase() {
    ...
    try {
        ...
        console.log('✅ Firebase Auth & Firestore 초기화 완료');
...
// 즉시 Firebase 초기화 시도 (더 빠른 초기화)
console.log('Firebase 초기화 시작 (즉시)');

// 즉시 초기화 시도
(async () => {
    try {
        await waitForFirebaseAndInitialize();
        console.log('✅ Firebase 초기화 완료 (즉시)');
    } catch (error) {
        console.warn('⚠️ Firebase 즉시 초기화 실패, DOM 로드 후 재시도:', error);
...
async function waitForFirebaseAndInitialize() {
    ...
    for (let i = 0; i < 200; i++) { // 10초 대기
        ...
        // 진행 상황 로그 (1초마다)
        if (i % 20 === 0 && i > 0) {
            console.log(`⏳ Firebase SDK 로드 대기 중... (${i/20}초 경과)`);
        }
